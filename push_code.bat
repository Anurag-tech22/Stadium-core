@echo off
echo ==================================================
echo   PHOENIX STADIUM - VERIFY TESTS AND PUSH TO GITHUB
echo ==================================================
echo.
...
if exist .venv\Scripts\ruff.exe (
    ...
    :: Check if ruff is installed globally, install it if not
...
if exist .venv\Scripts\pytest.exe (
    ...
    :: Check if pytest is installed globally, install it if not
    python -m pip show pytest >nul 2>&1
    if %ERRORLEVEL% neq 0 (
        echo Installing pytest and dependencies globally...
...
git add -A
git commit -m "fix: resolve all Ruff linter warnings and align with Rank 1 and 2 standards"
echo.
